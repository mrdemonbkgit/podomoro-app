...
export const SoundsPanel = ({ isOpen, onClose, isDark }: SoundsPanelProps) => {
  ...
  const { settings: soundSettings, setSound, removeSound, clearAll, loadPreset } = useAmbientSounds();
  ...
  const handlePresetClick = (presetId: string) => {
    ...
    setTimeout(() => {
      ...
      loadPreset(preset.sounds.map(({ soundId, volume }) => ({ id: soundId, volume })));
    ...
    <>
      ...
      >
        ...
        <div className={`sticky top-0 ${isDark ? 'bg-gray-900/90' : 'bg-white/90'} backdrop-blur-lg border-b ${isDark ? 'border-white/10' : 'border-gray-200'} p-6 z-10`}>
          ...
          {/* Presets */}
          <div className="mb-4">
            <h3 className={`text-sm font-semibold ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-2`}>
              Quick Presets
            </h3>
                    ...
                    {isActive && (
                      <div className="flex items-center gap-3 animate-slide-up">
                        ...
                        <input
                          ...
                          className="flex-1"
                        />
                        <span className={`text-sm font-medium w-12 text-right ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                          {volume}%
                        </span>
                      </div>
                    )}
