 ...
 */
export async function resetMainStreak(
  userId: string,
  previousSeconds: number
): Promise<Streaks> {
  try {
    logger.debug('🔄 resetMainStreak START (TRANSACTION):', {
      userId,
      previousSeconds,
    });
    ...
    const result = await runTransaction(db, async (transaction) => {
      ...
      if (currentJourneyId) {
        ...
        const journeyRef = doc(
          db,
          getDocPath.journey(userId, currentJourneyId)
        );
      ...
      const newJourneyRef = doc(
        collection(db, COLLECTION_PATHS.journeys(userId))
      );
      const newJourney = {
        startDate: now,
        endDate: null,
